...
module.exports = function (task, exec, done) {
	...
	function handleResult(err, result) {
		if (err) {
			grunt.fail.fatal("Error running New-WebAppPool");
			return;
		}
	...
	args.push("{ Import-Module WebAdministration; New-WebAppPool " + argUtil.getArgFlags().join(" ") + " }");
	args.push(handleResult);
	exec.apply(null, args);
};

module.exports.description = "Creates a new IIS application pool.";
