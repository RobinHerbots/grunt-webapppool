...
module.exports = function (task, exec, done) {
    ...
    function handleResult(err, result) {
        if (err) {
            grunt.fail.fatal('Error running Remove-WebAppPool');
            return;
        }
    ...
    args.push('{ Import-Module WebAdministration; Remove-WebAppPool ' + argUtil.getArgFlags().join(' ') + ' }');
    args.push(handleResult);
    exec.apply(null, args);
};

module.exports.description = 'Removes an application pool from IIS.';
